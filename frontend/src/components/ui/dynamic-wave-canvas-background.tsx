...
/**
 * Full-viewport animated wave background (canvas).
 * @see https://21st.dev/r/minhxthanh/dynamic-wave-canvas-background
 */
export default function HeroWave() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      if (!alive || !imageData || !data) return
  ...
  return (
    <canvas
      ref={canvasRef}
      className="pointer-events-none fixed inset-0 z-0 h-full min-h-svh w-full"
      aria-hidden
    />
  )
}
